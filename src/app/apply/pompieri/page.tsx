...
import { Flame, Send, CheckCircle, ArrowLeft } from "lucide-react";
...
const questions = [
  ...
  { id: "why_firefighter", label: "De ce vrei să faci parte din Pompieri?", type: "textarea", required: true },
  { id: "scenario1", label: "Scenariu: Ești chemat la un accident rutier. Ce faci?", type: "textarea", required: true },
...
export default function FirefighterApplication() {
  ...
  if (isSubmitted) {
    return (
      <div className="min-h-screen pt-24 pb-12 px-4 relative overflow-hidden">
        <FloatingElements count={6} colors={["#ef4444", "#f97316", "#f59e0b"]} />
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-4 relative">
      <FloatingElements count={5} colors={["#ef4444", "#f97316"]} />
      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        >
          ...
          <TiltCard tiltAmount={3} glowColor="rgba(239, 68, 68, 0.2)">
            <div className="p-8 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm">
              <div className="text-center mb-8">
                <motion.div
                  className="w-20 h-20 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center"
                  whileHover={{ scale: 1.1, rotate: 5 }}
                  transition={{ type: "spring", stiffness: 300 }}
                >
                  <Flame className="w-10 h-10 text-white" />
                </motion.div>
                <h1 className="text-3xl font-bold text-white mb-2">Aplică pentru Pompieri</h1>
                <p className="text-white/60">
                  Completează formularul de mai jos pentru a aplica în Departamentul de Pompieri
                </p>
              </div>
              ...
              >
                  ...
                  <motion.div key={q.id} variants={itemVariants}>
                    ...
                    {q.type === "textarea" ? (
                      <Textarea
                        ...
                        className="bg-white/5 border-white/10 text-white placeholder:text-white/40 min-h-[100px] focus:border-red-500/50 focus:ring-red-500/20 transition-all"
                        required={q.required}
                      />
                    ) : (
                      <Input
                        ...
                        value={formData[q.id] || ""}
                        onChange={(e) => handleChange(q.id, e.target.value)}
                        className="bg-white/5 border-white/10 text-white placeholder:text-white/40 focus:border-red-500/50 focus:ring-red-500/20 transition-all"
                        required={q.required}
                      />
                    )}
                  </motion.div>
                ...
                <motion.div variants={itemVariants}>
                  <Button
                    ...
                    className="w-full bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white py-6 relative overflow-hidden group"
                  >
                    <motion.div
